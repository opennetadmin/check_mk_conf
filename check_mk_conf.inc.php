...
order BY h.id";
    ...
    while ($ca = $rs->FetchRow()) {
      // Capture the kobraapp tags
      if ( $ca['name'] == 'lifecycle' ) { $lifecycles[] = $ca['value']; }
      // build check_mk tag list from custom attributes like "ona-<cakey>-<cavalue>"
      $cmkcalist[$ca['fqdn']]['calist']="{$cmkcalist[$ca['fqdn']]['calist']}|ona-{$ca['name']}-{$ca['value']}";
...
";
    foreach ($calist as $catype => $ca) {
      $text .= "  ('ona-{$catype}', u'ONA Tag {$catype}', [\n";
      $ca=array_unique($ca);
      foreach ($ca as $cavalue) {
        $text .= "  ('ona-{$catype}-".strtolower($cavalue)."', u'ona-{$catype}-".strtolower($cavalue)."', []),\n";
      }
      $text .= "  ]),\n";
    }
    $text .= "]\n";
