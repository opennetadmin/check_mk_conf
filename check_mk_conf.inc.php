...
and models.manufacturer_id = manufacturers.id
and {$ca_type_ignore}
order BY h.id";
...
order BY h.id";
    ...
    while ($ca = $rs->FetchRow()) {
      ...
      $site = $ca['site'];
      $manuf = strtolower(preg_replace("/[ ]/", '_', $ca['manufacturer']));
...
GROUP BY h.id";
      ...
      $cmkcalist[$ca['fqdn']]['cattributes']['manufacturer'] = $manuf;
...
";
  ...
  if (isset($options['wato_host_tags'])) {
    ...
    list($status, $rows, $mnfs) = db_get_records($onadb, 'manufacturers', 'id > 0');
    foreach ($mnfs as $mnf) {
      $mnflist[]=$mnf['name'];
    }
...
wato_host_tags += [
";
    // Set up a list of all ONA manufacturers
    $text .= "  ('ona-manufacturers', u'OpenNetAdmin Tags/manufacturers', [\n";
    $text .= "    (None, u'Undef', []),\n";
    sort($mnflist, SORT_NATURAL | SORT_FLAG_CASE);
    foreach (array_unique($mnflist) as $m) {
      $manuf = strtolower(preg_replace("/[ ]/", '_', $m));
      $text .= "    ('manufacturer-{$manuf}', u'{$manuf}', []),\n";
    }
    $text .= "  ]),\n";
...
TODO: idea. maybe generate host aliases???
...
extra_host_conf.setdefault('alias', []).extend([
  (u'prod01-api01', ['prod01-api01.example.com']),
